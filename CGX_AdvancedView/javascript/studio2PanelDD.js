        ...
        if (this.deletePanel) {
            ...
            if (Studio2.isSpecial(srcEl)) {
                if (Studio2.establishLocation(srcEl) == 'panels') {
                    ...
                    var panelDisplay = SUGAR.language.get('CGX_AdvancedView', 'LBL_NEW_PANEL') + ' ' + panelNumber;
                    document.getElementById("le_panelname_" + srcEl.id).childNodes[0].nodeValue = panelDisplay;
                    var params = {module: 'CGX_AdvancedView', action: 'saveProperty', view_module: view_module}
