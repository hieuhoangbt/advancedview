 ...
 * ******************************************************************************
 */
require_once ('modules/CGX_AdvancedView/MB/CGX_AdvancedViewModuleBuilder.php');
...
class CGX_AdvancedViewController extends SugarController {
    ...
    function action_editLayout() {
        ...
        if (empty($_REQUEST['view_package']) || $_REQUEST['view_package'] == "studio") {
            $sm = CGX_AdvancedViewStudioModuleFactory::getStudioModule($_REQUEST['view_module']);
    ...
    function action_SavePackage() {
        $mb = new CGX_AdvancedViewModuleBuilder();
    ...
    function action_saveLabels() {
        require_once 'modules/CGX_AdvancedView/parsers/parser.label.php';
    ...
    function action_SaveLabel() {
        if (!empty($_REQUEST['view_module']) && !empty($_REQUEST['labelValue'])) {
            ...
            $req = $_REQUEST;
            foreach (CGX_AdvancedViewModuleBuilder::getModuleAliases($_REQUEST['view_module']) as $key) {
                $req['view_module'] = $key;
    ...
    function action_saveSugarField() {
        ...
        $field->populateFromPost();
        require_once ('modules/CGX_AdvancedView/parsers/StandardField.php');
        $module = $_REQUEST['view_module'];
    ...
    function action_saveVisibility() {
        ...
        echo $json->encode(array(
            "visibility_editor_{$_REQUEST['fieldname']}" => array(
                "action" => "deactivate"
            )
        ));
    }
    ...
    function action_SaveRelationship() {
        ...
        if (empty($_REQUEST['view_package'])) {
            ...
            $mb = new CGX_AdvancedViewModuleBuilder();
            $module = & $mb->getPackageModule($_REQUEST['view_package'], $_REQUEST['view_module']);
            require_once 'modules/CGX_AdvancedView/parsers/relationships/UndeployedRelationships.php';
    ...
    function action_SaveDropDown() {
        require_once 'modules/CGX_AdvancedView/parsers/parser.dropdown.php';
    ...
    function action_DeleteField() {
        ...
        if (!isset($_REQUEST['view_package'])) {
            if (!empty($_REQUEST['name']) && !empty($_REQUEST['view_module'])) {
                ...
                require_once 'modules/CGX_AdvancedView/Module/CGX_AdvancedViewStudioModuleFactory.php';
                $module = CGX_AdvancedViewStudioModuleFactory::getStudioModule($moduleName);
            }
        } else {
            $mb = new CGX_AdvancedViewModuleBuilder();
            $module = & $mb->getPackageModule($_REQUEST['view_package'], $_REQUEST['view_module']);
    ...
    function action_deletegroup() {
         ...
         *       Delete group in db
         */
        $moduleName = $_REQUEST['module_setting'];
        $groupName = $_REQUEST['group_name'];

        $adv_view = new CGX_AdvancedView();
        $adv_view->deleteGroup($moduleName, $groupName);

        $groupPath = realpath("custom/modules/$moduleName/metadata/group/$groupName");
        if ($groupName != 'Default' && is_dir($groupPath)) {
            rmdir_recursive($groupPath);
        }
        $_REQUEST['view_module'] = $moduleName;
        $_REQUEST['view'] = 'layouts';
        $_REQUEST['removed'] = 1;
        $this->view = 'wizard';
    }
    ...
    /**
     * savetablesort
     * This method handles saving the current user's tabling sorting preferences.
     * It is called when
    ...
    function action_savetablesort() {
        ...
        if (!empty($current_user) && isset($_REQUEST['column']) && isset($_REQUEST['direction'])) {
            ...
            $current_user->setPreference('fieldsTableColumn', getJSONobj()->encode($val), 0, 'CGX_AdvancedView');
        }
    }
    ...
    /**
     * Action update sort to module
     */
    function action_sortUpdate() {
        if (isset($_POST)) {
            $module_name = $_REQUEST['module_name'];
            $sortedArr = $_REQUEST['sorted'];
            if (!empty($module_name) && !empty($sortedArr)) {
                for ($i = count($sortedArr); $i > 0; $i--) {
                    $adv_view_bean = new CGX_AdvancedView();
                    $adv_view_bean->retrieve_by_string_fields(array(
                        'name' => $module_name,
                        'group_folder' => $sortedArr[count($sortedArr) - $i]
                    ));
                    if ($adv_view_bean->id) {
                        $adv_view_bean->weight = $i;
                        $adv_view_bean->save();
                    } else {
                        $adv_view_bean->name = $module_name;
                        $adv_view_bean->group_folder = $sortedArr[count($sortedArr) - $i];
                        $adv_view_bean->group_name = $sortedArr[count($sortedArr) - $i];
                        $adv_view_bean->weight = $i;
                        $adv_view_bean->save();
                    }
                }
    ...
    /**     * ***************************************************************************************
     * Advanced View Application Controller Overwrite
     * **************************************************************************************** */
    public function loadBean() {
        $originalModule = $_REQUEST['original_adv_c'];
        if (!empty($GLOBALS['beanList'][$originalModule])) {
            $class = $GLOBALS['beanList'][$originalModule];
    ...
    /**
     * Overwrite process action
     * @assign GLOBALS module variable
     */
    public function process() {
        parent::process();
        $GLOBALS['module'] = $_REQUEST['original_adv_c'];
    }
