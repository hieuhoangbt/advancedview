...
class ViewDashlet extends ViewListView
{
    ...
    function display($preview = false)
    {
        ...
        $smarty = $this->constructSmarty($parser, boolval($bean->customer_implementation));
        ...
        if ($preview) {
            ...
            $ajax->addSection('center', translate('LBL_DASHLET'), $smarty->fetch("modules/CGX_AdvancedView/tpls/listView.tpl"));
    ...
    function constructAjax($group_name, $group_folder)
    {
        ...
        if ($this->fromModuleBuilder) {
            ...
            $ajax->addCrumb($_REQUEST ['view_package'], 'ModuleBuilder.getContent("module=CGX_AdvancedView&action=package&package=' . $this->editPackage . '")');
            $ajax->addCrumb($this->editModule, 'ModuleBuilder.getContent("module=CGX_AdvancedView&action=module&view_package=' . $this->editPackage . '&view_module=' . $this->editModule . '")');
            $ajax->addCrumb(translate('LBL_LAYOUTS', 'CGX_AdvancedView'), 'ModuleBuilder.getContent("module=CGX_AdvancedView&action=wizard&layouts=1&MB=1&view_package=' . $this->editPackage . '&view_module=' . $this->editModule . '")');
            $ajax->addCrumb(translate('LBL_DASHLET', 'CGX_AdvancedView'), 'ModuleBuilder.getContent("module=CGX_AdvancedView&action=wizard&view=dashlet&MB=1&view_package=' . $this->editPackage . '&view_module=' . $this->editModule . '")');
            ...
            $ajax->addCrumb(translate($this->editModule), 'ModuleBuilder.getContent("module=CGX_AdvancedView&action=wizard&view_module=' . $this->editModule . '")');
            $ajax->addCrumb(translate('LBL_LAYOUTS', 'CGX_AdvancedView'), 'ModuleBuilder.getContent("module=CGX_AdvancedView&action=wizard&layouts=1&view_module=' . $this->editModule . '")');
            ...
            $ajax->addCrumb(translate('LBL_DASHLET', 'CGX_AdvancedView'), 'ModuleBuilder.getContent("module=CGX_AdvancedView&action=wizard&view=dashlet&view_module=' . $this->editModule . '&group_name=' . $group_folder . '")');
            $ViewLabel = ($this->editLayout == MB_DASHLET) ? 'LBL_DASHLETLISTVIEW' : 'LBL_DASHLETSEARCHVIEW';
            $ajax->addCrumb(translate($ViewLabel, 'CGX_AdvancedView'), '');
        }
        return $ajax;
    }

    function constructSmarty($parser, $editDisabled)
    {
        ...
        $smarty->assign('module', 'CGX_AdvancedView');
