...
class ViewListView extends SugarView {
    ...
    function display($preview = false) {
        ...
        if ($preview) {
            ...
            $ajax->addSection('center', $this->translatedViewType, $smarty->fetch("modules/CGX_AdvancedView/tpls/listView.tpl"));
    ...
    function constructAjax($group_name, $group_folder) {
        ...
        if ($this->fromModuleBuilder) {
            ...
            $ajax->addCrumb($this->editModule, 'ModuleBuilder.getContent("module=CGX_AdvancedView&action=module&view_package=' . $_REQUEST['view_package'] . '&view_module=' . $_REQUEST['view_module'] . '")');
            if ($this->subpanel != "") {
                $ajax->addCrumb(translate('LBL_AVAILABLE_SUBPANELS', 'CGX_AdvancedView'), '');
                if ($this->subpanelLabel) {
                    ...
                    $this->translatedViewType = $subpanelLabel . "&nbsp;" . translate("LBL_SUBPANEL", "CGX_AdvancedView");
                } else {
                    $ajax->addCrumb($this->subpanel, '');
                    $this->translatedViewType = translate("LBL_SUBPANEL", "CGX_AdvancedView");
                }
            } else {
                $ajax->addCrumb(translate($layoutLabel, 'CGX_AdvancedView'), 'ModuleBuilder.getContent("module=CGX_AdvancedView&MB=true&action=wizard&view_module=' . $_REQUEST['view_module'] . '&view_package=' . $_REQUEST['view_package'] . '")');
                $ajax->addCrumb($translatedViewType, '');
            }
        } else {
            $ajax->addCrumb(translate('LBL_STUDIO', 'CGX_AdvancedView'), 'ModuleBuilder.getContent("module=CGX_AdvancedView&action=wizard")');
            ...
            if ($this->subpanel) {
                $ajax->addCrumb(translate('LBL_SUBPANELS', 'CGX_AdvancedView'), 'ModuleBuilder.getContent("module=CGX_AdvancedView&action=wizard&view=subpanels&view_module=' . $this->editModule . '")');
                if ($this->subpanelLabel) {
                    $subpanelLabel = $this->subpanelLabel;
                    // If the subpanel title has changed, use that for the label instead
                    if (!empty($_REQUEST['subpanel_title']) && $_REQUEST['subpanelLabel'] != $_REQUEST['subpanel_title'])
