...
//    }
    ...
    function display($preview = false)
    {
        ...
        if ($preview) {
            ...
            $ajax->addSection('center', translate('LBL_POPUP'), $smarty->fetch("modules/CGX_AdvancedView/tpls/listView.tpl"));
    ...
    function constructAjax($group_name, $group_folder)
    {
        ...
        if ($this->fromModuleBuilder) {
            ...
            $ajax->addCrumb(translate('LBL_LAYOUTS', 'ModuleBuilder'), 'ModuleBuilder.getContent("module=CGX_AdvancedView&action=wizard&layouts=1&MB=1&view_package=' . $this->editPackage . '&view_module=' . $this->editModule . '")');
            $ajax->addCrumb(translate('LBL_POPUP', 'ModuleBuilder'), 'ModuleBuilder.getContent("module=CGX_AdvancedView&action=wizard&view=popup&MB=1&view_package=' . $this->editPackage . '&view_module=' . $this->editModule . '&group_name=' . $_REQUEST['group_name'] . '")');

            $ViewLabel = ($this->editLayout == MB_POPUPLIST) ? 'LBL_POPUPLISTVIEW' : 'LBL_POPUPSEARCH';
            $ajax->addCrumb(translate($ViewLabel, 'ModuleBuilder'), '');
        } else {
            $ajax->addCrumb(translate($this->editModule), 'ModuleBuilder.getContent("module=CGX_AdvancedView&action=module&view_module=' . $this->editModule . '")');
            $ajax->addCrumb(translate('LBL_LAYOUTS', 'ModuleBuilder'), 'ModuleBuilder.getContent("module=CGX_AdvancedView&action=wizard&layouts=1&view_module=' . $this->editModule . '")');
            ...
            $ajax->addCrumb(translate('LBL_POPUP', 'ModuleBuilder'), 'ModuleBuilder.getContent("module=CGX_AdvancedView&action=wizard&view=popup&view_module=' . $this->editModule . '&group_name=' . $group_folder . '")');
            $ViewLabel = ($this->editLayout == MB_POPUPLIST) ? 'LBL_POPUPLISTVIEW' : 'LBL_POPUPSEARCH';
            $ajax->addCrumb(translate($ViewLabel, 'ModuleBuilder'), '');
        }
        return $ajax;
