...
if (!empty($originalModule)) {
     ...
     */
    $customPath = 'custom/modules/' . $originalModule . '/views/view.list.php';
    $modulePath = 'modules/' . $originalModule . '/views/view.list.php';
    if (file_exists($customPath)) {
        require_once $customPath;
        $parentClassName = $originalModule . 'ViewList';
...
eval("class ParentViewList extends $parentClassName{}");

class CustomViewList extends ParentViewList
{
    ...
    {
        if(get_parent_class('ParentViewList') != 'SugarView'){
        ...
        if (!is_admin($current_user) && !empty($roles)) {
            ...
            if ($groupOfCurrentUser != false) {
                ...
                $groupMetdata = "custom/modules/{$_REQUEST['original_adv_c']}/metadata/group/$group_folder/{$this->type}viewdefs.php";
    ...
    {
        parent::display();
        $_REQUEST['module'] = $_REQUEST['original_adv_c'];
