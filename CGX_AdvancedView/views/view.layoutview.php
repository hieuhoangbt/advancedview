...
class ViewLayoutView extends SugarView
{
    ...
    function __construct()
    {
        ...
        if ($this->fromModuleBuilder) {
            ...
            $this->sm = CGX_AdvancedViewStudioModuleFactory::getStudioModule($this->editModule);
    ...
    function display($preview = false)
    {
        ...
        if ($preview) {
            $smarty->assign('layouttitle', translate('LBL_LAYOUT_PREVIEW', 'CGX_AdvancedView'));
        } else {
            $smarty->assign('layouttitle', translate('LBL_CURRENT_LAYOUT', 'CGX_AdvancedView'));
            ...
            $buttons = array();
            if (!$this->fromModuleBuilder && boolval($bean->customer_implementation) == false) {
                $buttons[] = array(
                    'id' => 'saveBtn',
            ...
            } else if(boolval($bean->customer_implementation) == false){
                $buttons[] = array(
                    'id' => 'saveBtn',
            ...
            if (($this->editLayout == MB_DETAILVIEW || $this->editLayout == MB_QUICKCREATE) && boolval($bean->customer_implementation) == false) {
                $buttons[] = array(
                    'id' => 'copyFromEditView',
        ...
        $smarty->assign('group_name', str_replace(' ', '', trim($group_name)));
        ...
        if (isset($labels[strtolower($this->editLayout)]))
            $translatedViewType = translate($labels[strtolower($this->editLayout)], 'CGX_AdvancedView');
        else
        if (isset($this->sm)) {
        ...
        if ($this->fromModuleBuilder) {
            ...
            $ajax->addCrumb($this->package, 'ModuleBuilder.getContent("module=CGX_AdvancedView&action=package&package=' . $this->package . '")');
            $ajax->addCrumb($this->editModule, 'ModuleBuilder.getContent("module=CGX_AdvancedView&action=module&view_package=' . $this->package . '&view_module=' . $this->editModule . '")');
            $ajax->addCrumb(translate($layoutLabel, 'ModuleBuilder'), 'ModuleBuilder.getContent("module=CGX_AdvancedView&MB=true&action=wizard&view=' . $layoutView . '&view_module=' . $this->editModule . '&view_package=' . $this->package . '")');
            $ajax->addCrumb($translatedViewType, '');
        } else {
            $ajax->addCrumb(translate('LBL_STUDIO', 'CGX_AdvancedView'), 'ModuleBuilder.main("studio")');
            $ajax->addCrumb($this->translatedEditModule, 'CGX_AdvancedView.getContent("module=CGX_AdvancedView&action=wizard&view_module=' . $this->editModule . '")');
            ...
            $ajax->addCrumb($bean->group_name, 'ModuleBuilder.getContent("module=CGX_AdvancedView&action=wizard&view=groups&view_module=' . $this->editModule . '&group_name=' . $group_name . '")');
            $ajax->addCrumb($translatedViewType, '');
        }
        ...
        $ajax->addSection('center', $translatedViewType, $smarty->fetch('modules/CGX_AdvancedView/tpls/layoutView.tpl'));
        if ($preview) {
            echo $smarty->fetch('modules/CGX_AdvancedView/tpls/Preview/layoutView.tpl');
        } else {
