...
class ViewModule extends SugarView
{
    ...
    function display()
    {
        ...
        $types = (empty($module_name)) ? CGX_AdvancedViewMBModule::getTypes() : $this->mbModule->mbvardefs->templates;

        foreach ($types as $type => $definition) {
            $translated_type[$type] = translate('LBL_TYPE_' . strtoupper($type), 'CGX_AdvancedView');
        }
        ...
        $ajax->addCrumb(' ' . $package->name, 'ModuleBuilder.getContent("module=CGX_AdvancedView&action=package&package=' . $package->name . '")');
        if (empty($module_name))
            $module_name = translate('LBL_NEW_MODULE', 'CGX_AdvancedView');
        $ajax->addCrumb($module_name, '');
        $html = $smarty->fetch('modules/CGX_AdvancedView/tpls/MBModule/module.tpl');
        if (!empty($_REQUEST['action']) && $_REQUEST['action'] == 'SaveModule')
            $html .="<script>ModuleBuilder.treeRefresh('CGX_AdvancedView')</script>";
