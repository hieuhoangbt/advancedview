...
require_once 'modules/CGX_AdvancedView/parsers/views/AbstractMetaDataImplementation.php';
require_once 'modules/CGX_AdvancedView/parsers/views/MetaDataImplementationInterface.php';
require_once 'modules/CGX_AdvancedView/parsers/views/ListLayoutMetaDataParser.php';
require_once 'modules/CGX_AdvancedView/parsers/views/GridLayoutMetaDataParser.php';
require_once 'modules/CGX_AdvancedView/parsers/views/PopupMetaDataParser.php';
require_once 'modules/CGX_AdvancedView/Module/CGX_AdvancedViewStudioModule.php';
...
class DeployedMetaDataImplementation extends AbstractMetaDataImplementation implements MetaDataImplementationInterface
{
    ...
    function __construct($view, $moduleName)
    {
        ...
        $sm = CGX_AdvancedViewStudioModuleFactory::getStudioModule($moduleName);
        ...
        if ($loaded === null) {
            switch ($view) {
                ...
                case MB_DASHLET:
                    ...
                    $needSave = false;
                    // Custom get dashlet metadata from group
                    $extend_path = "";
                    if(!empty($group) && $group != 'Default') {
                        $extend_path = "group/{$group}/";
                    }
                    // End custom
                    if (file_exists("custom/modules/{$moduleName}/metadata/{$extend_path}" . basename($this->_sourceFilename)) && !isset($_REQUEST['clone_group'])) {
                        $loaded = $this->_loadFromFile("custom/modules/{$moduleName}/metadata/{$extend_path}" . basename($this->_sourceFilename));
            ...
            foreach (array(
                ...
                if (file_exists($sourceFilename)) {
                    ...
                    if (null !== $layout && isset($layout[GridLayoutMetaDataParser::$variableMap[$v]])) {
                        $layout = array(
                            GridLayoutMetaDataParser::$variableMap[MB_QUICKCREATE] => $layout[GridLayoutMetaDataParser::$variableMap[$v]]
                        );
                        break;
                    }
                }
    ...
    public static function getFileName($view, $moduleName, $type = MB_CUSTOMMETADATALOCATION)
    {
        ...
        $file_name = $pathMap[$type] . 'modules/' . $moduleName . '/metadata/' . $filenames[$view] . '.php';
        if (! empty($group_folder) && $group_folder != 'Default') {
            $file_name = $pathMap[$type] . 'modules/' . $moduleName . "/metadata/group/{$group_folder}/" . $filenames[$view] . '.php';
        }
        return $file_name;
    }
