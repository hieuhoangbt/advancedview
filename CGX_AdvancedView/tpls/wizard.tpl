...
<div style="float:left; left:-100px; display: hidden;">&nbsp;
	...
	<style type='text/css'>
		...
		.backButton {position:absolute; left:10px; top:35px}
                #cstm_group_ins{width:10px; height: 16px; display: inline-block; position: relative;}
                #cstm_group_ins a{width: 100%; position: absolute;}
                #cstm_group_ins a.cstm_up{top: -5px;}
                #cstm_group_ins a.cstm_down{bottom: -5px;}
                #cstm_group_ins img{width: 100%;}
	</style>
    {/literal}
	...
	ModuleBuilder.helpSetup('studioWizard','{$defaultHelp}');
        {if isset($removed)}
            ModuleBuilder.treeRefresh('{$mod}');
        {/if}
	</script>
</div>
{include file='modules/CGX_AdvancedView/tpls/assistantJavascript.tpl'}
