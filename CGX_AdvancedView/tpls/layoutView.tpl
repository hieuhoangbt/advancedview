...
{if $disable_layout}
<span class='required'>
{sugar_translate label="LBL_SYNC_TO_DETAILVIEW_NOTICE" module="CGX_AdvancedView"}
</span>
{/if}
<table id='layoutEditorButtons' cellspacing='2'>
	...
	<td><input type="checkbox" {if $syncDetailEditViews}checked="true"{/if} id="syncCheckbox" onclick="document.forms.prepareForSave.sync_detail_and_edit.value=this.checked">
	   {sugar_translate label="LBL_SYNC_TO_DETAILVIEW" module="CGX_AdvancedView"}&nbsp;{sugar_help text=$mod.LBL_SYNC_TO_DETAILVIEW_HELP}
	</input></td>
	{/if}
    </tr>
</table>
...
{foreach from=$layout item='panel' key='panelid'}
    ...
    <div class='le_panel' id='{$idCount}'>
        ...
        &nbsp;&nbsp;{sugar_translate label="LBL_TABDEF_TYPE" module="CGX_AdvancedView"}&nbsp;{sugar_help text=$mod.LBL_TABDEF_TYPE_OPTION_HELP}:
        {if $idCounter == 1}
        ...
        <select id="le_paneltype_select_{$idCount}" onchange="document.forms.prepareForSave.tabDefs_{$panelid}_newTab.value=this.value; showHideBox(this.value, {$idCount}, '{$panelid}', '{$firstpanelid}', {$firstpanelidcount});"
                title="{sugar_translate label="LBL_TABDEF_TYPE_HELP" module="CGX_AdvancedView"}">
          <option value="0" {if $tabDefs[$panel_upper].newTab == false}selected="selected"{/if}>{sugar_translate label="LBL_TABDEF_TYPE_OPTION_PANEL" module="CGX_AdvancedView"}</option>
          {if $disable_tabs != true}
          <option value="1" {if $tabDefs[$panel_upper].newTab == true}selected="selected"{/if}>{sugar_translate label="LBL_TABDEF_TYPE_OPTION_TAB" module="CGX_AdvancedView"}</option>
          {/if}
        </select>
        </span>
        ...
        &nbsp;{sugar_translate label="LBL_TABDEF_COLLAPSE" module="CGX_AdvancedView"}?
        <input type="checkbox" title="{sugar_translate label="LBL_TABDEF_COLLAPSE_HELP" module="CGX_AdvancedView"}" {if $tabDefs[$panel_upper].panelDefault == "collapsed"}checked="checked"{/if}
...
var editPanelProperties = function (panelId, view) {
	...
	var params = "module=CGX_AdvancedView&action=editProperty&view_module=" + encodeURIComponent(ModuleBuilder.module)
                ...
                + "&title_label=" + encodeURIComponent(SUGAR.language.get("CGX_AdvancedView", "LBL_LABEL_TITLE")) + "&value_label=" + encodeURIComponent(value_label);
...
var editFieldProperties = function (idCount, label) {ldelim}
	...
	var title_label = '{sugar_translate label="LBL_LABEL_TITLE" module="CGX_AdvancedView"}';
	var title_tabindex = '{sugar_translate label="LBL_TAB_ORDER" module="CGX_AdvancedView"}';

	ModuleBuilder.getContent(
	  	'module=CGX_AdvancedView&action=editProperty'
...
var editDisabled = parseInt({$EDIT_DISABLED});
{literal}
if(editDisabled != 1){
    Studio2.init();
}else{
