...
class CGX_AdvancedViewMBPackageTree {
    function __construct() {
        $this->tree = new Tree('package_tree');
        $this->tree->id = 'package_tree';
        $this->mb = new CGX_AdvancedViewModuleBuilder();
    ...
    function populateTree($nodes, &$parent) {
        foreach ($nodes as $node) {
            ...
            if (isset($node['isGroup']) && $node['isGroup'] == true) {
                $yn->set_property('is_group', 1);
                $yn->set_property('group_folder', $node['groupFolder']);
            }
