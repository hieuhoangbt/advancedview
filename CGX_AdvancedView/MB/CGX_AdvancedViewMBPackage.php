...
require_once('modules/CGX_AdvancedView/MB/CGX_AdvancedViewMBModule.php');

class CGX_AdvancedViewMBPackage{
    var $name;
     ...
     */
    function CGX_AdvancedViewMBPackage($name){
...
function buildInstall($path){
    ...
    function getNodes(){
        $this->loadModules();
        $node = array('name'=>$this->name, 'action'=>'module=CGX_AdvancedView&action=package&package=' . $this->name, 'children'=>array());
        foreach(array_keys($this->modules) as $module){
